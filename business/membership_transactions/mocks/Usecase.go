...
func (_m *Usecase) GetAll(date time.Time, status string, idUser uint, page int) ([]membership_transactions.Domain, int, int, int64, error) {
	ret := _m.Called(date, status, idUser, page)

	var r0 []membership_transactions.Domain
	if rf, ok := ret.Get(0).(func(time.Time, string, uint, int) []membership_transactions.Domain); ok {
