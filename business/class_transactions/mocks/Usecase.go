...
// GetAll provides a mock function with given fields: date, status, idUser, page
func (_m *Usecase) GetAll(date time.Time, status string, idUser uint, page int) ([]class_transactions.Domain, int, int, int64, error) {
	ret := _m.Called(date, status, idUser, page)

	var r0 []class_transactions.Domain
	if rf, ok := ret.Get(0).(func(time.Time, string, uint, int) []class_transactions.Domain); ok {
		r0 = rf(date, status, idUser, page)
	} else {
		if ret.Get(0) != nil {
	...
	var r1 int
	if rf, ok := ret.Get(1).(func(time.Time, string, uint, int) int); ok {
		r1 = rf(date, status, idUser, page)
	} else {
		r1 = ret.Get(1).(int)
	}

	var r2 int
	if rf, ok := ret.Get(2).(func(time.Time, string, uint, int) int); ok {
		r2 = rf(date, status, idUser, page)
	} else {
		r2 = ret.Get(2).(int)
	}

	var r3 int64
	if rf, ok := ret.Get(3).(func(time.Time, string, uint, int) int64); ok {
		r3 = rf(date, status, idUser, page)
	} else {
		r3 = ret.Get(3).(int64)
	}

	var r4 error
	if rf, ok := ret.Get(4).(func(time.Time, string, uint, int) error); ok {
		r4 = rf(date, status, idUser, page)
	} else {
		r4 = ret.Error(4)
	}
...
func (_m *Usecase) UpdateStatusToFailed(idTransaction uint) (class_transactions.Domain, error) {
	ret := _m.Called(idTransaction)
